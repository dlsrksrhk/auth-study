...
class OAuthAccessTokenJpaEntity {
    ...
    @Column(name = "access_token_hash", nullable = false, updatable = false)
    ...
    static OAuthAccessTokenJpaEntity from(OAuthAccessToken token) {
        ...
        entity.accessTokenHash = token.accessTokenHash();
        entity.jti = token.jti();
        entity.audience = token.audience();
    ...
    void updateFrom(OAuthAccessToken token) {
        revokedAt = token.revokedAt();
    }

    OAuthAccessToken toDomain() {
        return OAuthAccessToken.restore(id, authorizationId, accessTokenHash, jti,
