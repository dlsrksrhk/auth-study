...
import com.sweet.authstudy.hr.company.domain.*;
import com.sweet.authstudy.hr.position.domain.*;
import com.sweet.authstudy.hr.user.domain.*;
...
abstract class OAuthAdminTestSupport {
    ...
    @Autowired OAuthClientRepository clients;
    @Autowired OAuthSubjectRepository subjects;
    @Autowired OAuthConsentRepository consents;
    @Autowired OAuthProtocolEventRepository events;
    ...
    String path() { return "/api/v1/admin/companies/"+company.code()+"/oauth-clients/"+client.clientId(); }
    void consent() { consents.save(OAuthConsent.create(account.id(), client.id(), Set.of("openid"), now)); }
