...
class OAuthConsentAdminControllerIntegrationTest extends OAuthAdminTestSupport {
    @Test void consent_page_exposes_only_public_subject_and_scopes() throws Exception {
        consent();
    ...
    @Test void tenant_and_missing_resource_errors_are_distinct() throws Exception {
    ...
    @Test void consent_delete_revokes_grants_and_refresh_and_is_audited() throws Exception {
        consent(); String id=java.util.UUID.randomUUID().toString(); grant(id);
        var originalAccount=account; var originalSubject=subject; var originalClient=client;
        ...
        consent(); String otherSubjectGrant=java.util.UUID.randomUUID().toString(); grant(otherSubjectGrant);
        account=originalAccount; subject=originalSubject;
        client=client(company,"same-tenant-"+java.util.UUID.randomUUID());
        consent(); String otherClientGrant=java.util.UUID.randomUUID().toString(); grant(otherClientGrant);
    ...
    @Test void client_revocation_preserves_consent() throws Exception {
        consent(); String id=java.util.UUID.randomUUID().toString(); grant(id);
    ...
    @Test void consent_query_filters_mismatched_tenant_rows_and_other_clients() throws Exception {
        consent();
