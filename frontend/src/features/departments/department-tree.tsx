...
export function DepartmentTree({ companyCode }: { companyCode: string }) {
  ...
  useEffect(() => {
    ...
    departmentApi.listAll(canonicalCode, controller.signal).then((items) => {
      ...
      setDepartments([]); setSelectedId(null); setFocusedId(null);
  ...
  const focusAt = useCallback((index: number) => {
    ...
    setFocusedId(id); itemRefs.current.get(id)?.focus();
  }, [visible]);
  ...
  function toggle(node: TreeNode) {
    ...
    setExpanded((old) => { const next = new Set(old); if (collapsing) next.delete(node.id); else next.add(node.id); return next; });
    if (collapsing && (isDescendant(focusedId, node.id) || isDescendant(selectedId, node.id))) {
      setFocusedId(node.id); setSelectedId(node.id); itemRefs.current.get(node.id)?.focus();
    }
  }
  function keyDown(event: React.KeyboardEvent, node: TreeNode) {
    ...
    else if (event.key === "ArrowRight" && node.children.length) { event.preventDefault(); if (!expanded.has(node.id)) setExpanded((old) => new Set(old).add(node.id)); else { setFocusedId(node.children[0].id); itemRefs.current.get(node.children[0].id)?.focus(); } }
    else if (event.key === "ArrowLeft") { event.preventDefault(); if (expanded.has(node.id) && node.children.length) toggle(node); else if (node.parentDepartmentId) { setFocusedId(node.parentDepartmentId); itemRefs.current.get(node.parentDepartmentId)?.focus(); } }
    else if (event.key === "Enter") { event.preventDefault(); setSelectedId(node.id); }
    else if (event.key === " ") { event.preventDefault(); setSelectedId(node.id); if (node.children.length) toggle(node); }
  }

  function renderNodes(nodes: TreeNode[], level: number): React.ReactNode {
    return nodes.map((node) => <li aria-expanded={node.children.length ? expanded.has(node.id) : undefined} aria-label={`${node.code} ${node.name}`} aria-level={level} aria-selected={selectedId === node.id} className="outline-none" key={node.id} onClick={(event) => { event.stopPropagation(); setSelectedId(node.id); setFocusedId(node.id); itemRefs.current.get(node.id)?.focus(); }} onKeyDown={(event) => keyDown(event, node)} ref={(element) => { if (element) itemRefs.current.set(node.id, element); else itemRefs.current.delete(node.id); }} role="treeitem" tabIndex={focusedId === node.id ? 0 : -1}>
      <div className={`flex items-center gap-2 rounded-md px-2 py-1.5 ${selectedId === node.id ? "bg-teal-50 text-teal-900" : "hover:bg-slate-50"}`} style={{ paddingLeft: `${(level - 1) * 20 + 8}px` }}>
        {node.children.length ? <button aria-label={`${node.code} ${expanded.has(node.id) ? "접기" : "펼치기"}`} className="size-6" onClick={(event) => { event.stopPropagation(); toggle(node); }} tabIndex={-1} type="button">{expanded.has(node.id) ? "▾" : "▸"}</button> : <span className="inline-block size-6" />}
  ...
  return <section aria-labelledby="department-title" className="mx-auto max-w-7xl">
    ...
    {form ? <DepartmentForm companyCode={canonicalCode} department={form.department} departments={departments} mode={form.mode} onConflict={() => { setNotice("다른 관리자가 수정해 최신 부서 정보를 다시 불러왔습니다."); restoreFocus.current = true; setReload((value) => value + 1); }} onOpenChange={(open) => { if (!open) { setForm(null); requestAnimationFrame(() => focusedId && itemRefs.current.get(focusedId)?.focus()); } }} onSaved={() => { restoreFocus.current = true; setReload((value) => value + 1); }} open /> : null}
  </section>;
}
