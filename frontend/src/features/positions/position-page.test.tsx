...
it("removes a deactivated position by re-fetching the current active filter", async () => {
  ...
  server.use(
    http.get("/api/v1/admin/companies/ACME/positions", ({ request }) => {
      ...
      listCalls += 1;
      return HttpResponse.json({ content: inactive ? [] : [position], page: 0, size: 20, totalElements: inactive ? 0 : 1, totalPages: inactive ? 0 : 1 });
    }),
    http.put("/api/v1/admin/companies/ACME/positions/EMPLOYEE", () => {
      inactive = true;
...
it("does not locally append a create result to a full server page", async () => {
  ...
  server.use(
    http.get("/api/v1/admin/companies/ACME/positions", ({ request }) => {
      expect(Object.fromEntries(new URL(request.url).searchParams)).toEqual({ page: "1", size: "20", sort: "displayOrder" });
      listCalls += 1;
