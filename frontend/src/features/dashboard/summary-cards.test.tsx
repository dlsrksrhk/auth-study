...
describe("SummaryCards", () => {
  ...
  it("선택한 회사의 서버 집계 수치를 표시한다", async () => {
    vi.mocked(dashboardApi.summary).mockResolvedValue({ activeUsers: 7, departments: 3, lockedUsers: 2, resignedUsers: 1 });
  ...
  it("이전 회사의 늦은 응답을 무시한다", async () => {
    let resolveOld!: (value: { activeUsers: number; departments: number; lockedUsers: number; resignedUsers: number }) => void;
