...
export function UserTable({ companyCode }: { companyCode: string }) {
  ...
  return <section aria-labelledby="user-title" className="mx-auto max-w-7xl"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold text-teal-700">{canonicalCode}</p><h1 className="mt-2 text-3xl font-semibold" id="user-title">사용자 관리</h1><p className="mt-2 text-sm text-slate-600">이름, 코드, 사번 검색과 사용자 생명주기를 관리합니다.</p></div><Button onClick={() => setCreating(true)}>사용자 생성</Button></div>
    <div className="mt-6 rounded-xl border bg-white shadow-sm"><div className="grid gap-4 border-b p-4 sm:grid-cols-[1fr_12rem_12rem_8rem]"><UserSearch initialValue={querySearch} key={querySearch} onCommit={(value) => replaceQuery({ search: value || null, page: null })} /><Filter id="user-status" label="상태" value={status ?? ""} options={[["", "전체"], ["PENDING", "대기"], ["ACTIVE", "활성"], ["LOCKED", "잠금"], ["RESIGNED", "퇴사"]]} onChange={(value) => replaceQuery({ status: value || null, page: null })} /><Filter id="user-sort" label="정렬" value={sort} options={[["code", "코드"], ["employeeNumber", "사번"], ["name", "이름"], ["status", "상태"]]} onChange={(value) => replaceQuery({ sort: value, page: null })} /><Filter id="user-size" label="페이지 크기" value={String(size)} options={[["20", "20개"], ["50", "50개"], ["100", "100개"]]} onChange={(value) => replaceQuery({ size: value, page: null })} /></div>
...
function UserSearch({ initialValue, onCommit }: { initialValue: string; onCommit(value: string): void }) {
  const [value, setValue] = useState(initialValue);
  useEffect(() => { if (value === initialValue) return; const timeout = window.setTimeout(() => onCommit(value.trim()), 300); return () => window.clearTimeout(timeout); }, [initialValue, onCommit, value]);
  return <div className="space-y-2"><Label htmlFor="user-search">사용자 검색</Label><Input id="user-search" placeholder="이름, 코드 또는 사번" value={value} onChange={(event) => setValue(event.target.value)} /></div>;
}
