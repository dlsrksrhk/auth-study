...
export function UserTable({ companyCode }: { companyCode: string }) {
  ...
  const replaceQuery = useCallback((changes: Record<string, string | null>) => { const next = new URLSearchParams(latestCanonicalQuery.current); Object.entries(changes).forEach(([key, value]) => { if (!value || (key === "page" && value === "0") || (key === "size" && value === "20") || (key === "sort" && value === "code")) next.delete(key); else next.set(key, value); }); latestCanonicalQuery.current = next.toString(); replace(`${pathname}${next.size ? `?${next}` : ""}`, { scroll: false }); }, [pathname, replace]);
  useEffect(() => { latestCanonicalQuery.current = canonicalQuery; }, [canonicalQuery]);
  ...
  useEffect(() => {
    ...
    Promise.all([userApi.list(canonicalCode, { page, size, sort, search: querySearch || undefined, status }, controller.signal), positionApi.listAll(canonicalCode, true, controller.signal)]).then(([next, nextPositions]) => { if (id !== requestId.current) return; setResult(next); setPositions(nextPositions); if (next.totalPages > 0 && page >= next.totalPages) replaceQuery({ page: String(next.totalPages - 1) }); }).catch((cause) => { if (id !== requestId.current || (cause instanceof DOMException && cause.name === "AbortError")) return; setError(isApiProblemError(cause) ? `${cause.detail ?? cause.title} (추적 ID: ${cause.traceId})` : cause instanceof Error ? cause.message : "사용자 목록을 불러오지 못했습니다."); }).finally(() => { if (id === requestId.current) setLoading(false); });
  ...
  return <section aria-labelledby="user-title" className="mx-auto max-w-7xl"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold text-teal-700">{canonicalCode}</p><h1 className="mt-2 text-3xl font-semibold" id="user-title">사용자 관리</h1><p className="mt-2 text-sm text-slate-600">이름, 코드, 사번 검색과 사용자 생명주기를 관리합니다.</p></div><Button disabled={Boolean(secretOperations.pending)} onClick={() => setCreating(true)}>사용자 생성</Button></div>
    <div className="mt-6 rounded-xl border bg-white shadow-sm"><div className="grid gap-4 border-b p-4 sm:grid-cols-[1fr_12rem_12rem_8rem]"><UserSearch queryValue={querySearch} onCommit={(value) => replaceQuery({ search: value || null, page: null })} /><Filter id="user-status" label="상태" value={status ?? ""} options={[["", "전체"], ["PENDING", "대기"], ["ACTIVE", "활성"], ["LOCKED", "잠금"], ["RESIGNED", "퇴사"]]} onChange={(value) => replaceQuery({ status: value || null, page: null })} /><Filter id="user-sort" label="정렬" value={sort} options={[["code", "코드"], ["employeeNumber", "사번"], ["name", "이름"], ["status", "상태"]]} onChange={(value) => replaceQuery({ sort: value, page: null })} /><Filter id="user-size" label="페이지 크기" value={String(size)} options={[["20", "20개"], ["50", "50개"], ["100", "100개"]]} onChange={(value) => replaceQuery({ size: value, page: null })} /></div>
...
function UserSearch({ queryValue, onCommit }: { queryValue: string; onCommit(value: string): void }) {
  ...
  useEffect(() => { latestCommit.current = onCommit; }, [onCommit]);
  useEffect(() => {
    if (lastSelfCommit.current === queryValue) { lastSelfCommit.current = null; return; }
    lastSelfCommit.current = null;
    if (timeout.current !== null) { window.clearTimeout(timeout.current); timeout.current = null; }
    if (input.current && input.current.value !== queryValue) input.current.value = queryValue;
  }, [queryValue]);
  useEffect(() => () => { if (timeout.current !== null) window.clearTimeout(timeout.current); }, []);
  return <div className="space-y-2"><Label htmlFor="user-search">사용자 검색</Label><Input defaultValue={queryValue} id="user-search" placeholder="이름, 코드 또는 사번" ref={input} onChange={(event) => {
    if (timeout.current !== null) window.clearTimeout(timeout.current);
    const value = event.currentTarget.value;
    timeout.current = window.setTimeout(() => { timeout.current = null; lastSelfCommit.current = value.trim(); latestCommit.current(value.trim()); }, 300);
  }} /></div>;
}
