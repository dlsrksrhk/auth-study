...
it("requires exactly one active primary membership and active position before activation", async () => {
  ...
  render(<MembershipEditor companyCode="ACME" departments={departments} memberships={[...memberships]}
                           positionActive={false} user={userRecord} onRefresh={() => undefined}/>);
...
it("switches primary atomically through one update request and keeps active history separate", async () => {
  ...
  render(<MembershipEditor companyCode="ACME" departments={departments} memberships={[...withPrimary, {
    ...memberships[0],
    id: 21,
    primary: false,
    endedAt: "2026-08-19T00:00:00Z"
  }]} positionActive user={{...userRecord, status: "ACTIVE"}} onRefresh={() => undefined}/>);
...
it("prevents ending the sole primary of an active user and surfaces HEAD conflict", async () => {
  ...
  let endCalls = 0;
  server.use(
      http.delete("/api/v1/admin/companies/ACME/users/U001/memberships/31", () => {
        endCalls += 1;
      ...
      http.put("/api/v1/admin/companies/ACME/users/U001/memberships/31", () => HttpResponse.json({
        ...
        title: "Conflict",
        detail: "Department already has an active head.",
        status: 409,
        code: "INVALID_STATE",
        traceId: "head-conflict",
        fieldErrors: [],
      }, {status: 409})),
  );
  render(<MembershipEditor companyCode="ACME" departments={departments} memberships={withPrimary} positionActive
                           user={{...userRecord, status: "ACTIVE"}} onRefresh={() => undefined}/>);
...
it("refreshes server state after an optimistic membership version conflict", async () => {
  ...
  server.use(http.put("/api/v1/admin/companies/ACME/users/U001/memberships/32", () => HttpResponse.json({
    ...
    title: "Conflict",
    detail: "Membership version does not match.",
    status: 409,
    code: "OPTIMISTIC_LOCK_CONFLICT",
    traceId: "membership-stale",
    fieldErrors: [],
  }, {status: 409})));
  render(<MembershipEditor companyCode="ACME" departments={departments}
                           memberships={[{...memberships[0], primary: true}, memberships[1]]} positionActive
                           user={{...userRecord, status: "ACTIVE"}} onRefresh={refresh}/>);
...
it("shows a creation password once and clears it permanently when the dialog closes", async () => {
  ...
  renderAdmin(<UserForm companyCode="ACME" onRefresh={() => undefined} open positions={[{
    id: 5,
    companyId: 7,
    code: "EMPLOYEE",
    name: "사원",
...
it("commits a Strict Mode UserForm create continuation exactly once", async () => {
  ...
  server.use(http.post("/api/v1/admin/companies/ACME/users", () => HttpResponse.json({
    user: userRecord,
    temporaryPassword: "StrictCreate123!"
  }, {status: 201})));
  render(<StrictMode><AdminSecretOperationProvider><UserForm companyCode="ACME" onRefresh={onRefresh} open positions={[{
    id: 5,
    companyId: 7,
    code: "EMPLOYEE",
    name: "사원",
    ...
    createdAt: "",
    updatedAt: ""
  }]} onOpenChange={onOpenChange}/></AdminSecretOperationProvider></StrictMode>);
  await user.type(screen.getByLabelText("사용자 코드"), "U001");
  await user.type(screen.getByLabelText("사번"), "E001");
  await user.type(screen.getByLabelText("이름"), "홍길동");
  await user.type(screen.getByLabelText("로그인 이메일"), "u001@acme.test");
...
it("commits a Strict Mode UserDetail reset continuation and refetch exactly once", async () => {
  ...
  let detailCalls = 0;
  server.use(
      http.get("/api/v1/admin/companies/ACME/users/U001", () => {
        detailCalls += 1;
        return HttpResponse.json(userRecord);
      }),
  ...
  render(<StrictMode><AdminSecretOperationProvider><UserDetail actorRoles={["SYSTEM_ADMIN"]} companyCode="ACME"
                                                               userCode="U001"/></AdminSecretOperationProvider></StrictMode>);
...
it("renders an XSS-shaped name as text and surfaces peer-admin 403 from role-sensitive controls", async () => {
  ...
  server.use(
      ...
      http.post("/api/v1/admin/companies/ACME/users/U001/temporary-password", () => HttpResponse.json({
        type: "about:blank",
        title: "Forbidden",
        detail: "관리자 계정은 시스템 관리자만 변경할 수 있습니다.",
        status: 403,
        code: "FORBIDDEN",
        traceId: "peer-admin-403",
        fieldErrors: []
      }, {status: 403})),
  );
...
it("shows exactly one truthful admin role action, confirms session revocation, and refetches", async () => {
  ...
  server.use(
      http.get("/api/v1/admin/companies/ACME/users/U001", () => {
        detailCalls += 1;
        return HttpResponse.json({...userRecord, roles: companyAdmin ? ["USER", "COMPANY_ADMIN"] : ["USER"]});
      }),
      http.get("/api/v1/admin/companies/ACME/positions", () => HttpResponse.json({
        content: [{
          id: 5,
      ...
      http.put("/api/v1/admin/companies/ACME/users/U001/admin-role", () => {
        grantCalls += 1;
        companyAdmin = true;
...
it("hides password reset from a company admin targeting a peer company admin", async () => {
  server.use(
      http.get("/api/v1/admin/companies/ACME/users/U001", () => HttpResponse.json({
        ...userRecord,
...
it("keeps an actual reset request and one-time secret across a detail route child switch", async () => {
  ...
  function Routes() {
    ...
    return <AdminSecretOperationProvider>
      <button type="button" onClick={() => setRoute("departments")}>부서 route로 이동</button>
      {route === "detail" ? <UserDetail actorRoles={["SYSTEM_ADMIN"]} companyCode="ACME" userCode="U001"/> :
          <p>부서 route 화면</p>}</AdminSecretOperationProvider>;
  }
...
it("loads every position and membership page before resolving current position, primary, and history", async () => {
  ...
  server.use(
      ...
      http.get("/api/v1/admin/companies/ACME/positions", ({request}) => {
        ...
        requests.push(`positions:${url.searchParams}`);
      ...
      http.get("/api/v1/admin/companies/ACME/users/U001/memberships", ({request}) => {
        const url = new URL(request.url);
        requests.push(`memberships:${url.searchParams}`);
        const page = Number(url.searchParams.get("page"));
        return HttpResponse.json(page === 0
            ? {content: [memberships[1]], page: 0, size: 100, totalElements: 101, totalPages: 2}
            : {
              content: [{...memberships[0], primary: true}, {
                ...memberships[1],
                id: 99,
                endedAt: "2026-08-19T00:00:00Z"
              }], page: 1, size: 100, totalElements: 101, totalPages: 2
...
it("closes a stale user form and refetches the latest profile/version after an optimistic conflict", async () => {
  ...
  server.use(
      http.get("/api/v1/admin/companies/ACME/users/U001", () => {
        detailCalls += 1;
        return HttpResponse.json(detailCalls === 1 ? userRecord : {...userRecord, name: "서버 최신 이름", version: 9});
      }),
      http.get("/api/v1/admin/companies/ACME/positions", () => HttpResponse.json({
        content: [{
          id: 5,
      ...
      http.put("/api/v1/admin/companies/ACME/users/U001", async ({request}) => {
        ...
        return HttpResponse.json({
          type: "about:blank",
          title: "Conflict",
          detail: "User version does not match.",
          status: 409,
          code: "OPTIMISTIC_LOCK_CONFLICT",
          traceId: "user-stale",
          fieldErrors: []
