...
const input = { code: "U001", employeeNumber: "E001", name: "홍길동", loginEmail: "u001@acme.test", phone: "010", hiredAt: "2026-08-20", workplace: "서울", profileImageUrl: "", positionCode: "EMPLOYEE" };
const createdUser = { id: 10, companyId: 7, code: "U001", employeeNumber: "E001", name: "홍길동", loginEmail: "u001@acme.test", roles: ["USER"], phone: "010", hiredAt: "2026-08-20", workplace: "서울", profileImageUrl: "", positionId: 5, status: "PENDING", version: 0, createdAt: "", updatedAt: "" };

beforeEach(() => { authSession.set("admin-token", "authenticated"); });

function Initiator({ kind }: { kind: "create" | "reset" }) {
  const operations = useAdminSecretOperations();
  return <button type="button" onClick={() => void (kind === "create" ? operations.createUser("ACME", input) : operations.resetPassword("ACME", "U001", "u001@acme.test"))}>작업 시작</button>;
}

function RouteHarness({ kind }: { kind: "create" | "reset" }) {
  const [route, setRoute] = useState<"user" | "department">("user");
  return <AdminSecretOperationProvider><button type="button" onClick={() => setRoute("department")}>부서 route로 이동</button>{route === "user" ? <Initiator kind={kind} /> : <p>부서 route</p>}</AdminSecretOperationProvider>;
}
...
it.each(["create", "reset"] as const)("keeps a committed %s secret operation alive across an admin child route switch", async (kind) => {
  ...
  server.use(http.post(path, async ({ request }) => {
    ...
    return HttpResponse.json(kind === "create" ? { user: createdUser, temporaryPassword: "RouteSafeCreate123!" } : { temporaryPassword: "RouteSafeReset123!" }, { status: kind === "create" ? 201 : 200 });
  }));
...
it("keeps the provider owner active after the Strict Mode effect lifecycle probe", async () => {
  ...
  render(<StrictMode><AdminSecretOperationProvider><Initiator kind="reset" /></AdminSecretOperationProvider></StrictMode>);
  await user.click(screen.getByRole("button", { name: "작업 시작" }));
...
it.each(["resolve", "reject"] as const)("ignores a delayed clipboard %s from a cleared secret after a new secret arrives", async (outcome) => {
  ...
  function ClipboardHarness() {
    const operations = useAdminSecretOperations();
    return <><button onClick={() => void operations.resetPassword("ACME", "U001", "first@acme.test")}>첫 재발급</button><button onClick={() => void operations.resetPassword("ACME", "U002", "second@acme.test")}>둘째 재발급</button></>;
  }
