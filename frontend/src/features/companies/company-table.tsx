      ...
      <section aria-labelledby="company-title" className="mx-auto max-w-7xl">
        ...
        {defaultPositionCheck?.status === "loading" ? (
            ...
            <Alert className="mt-6 border-teal-200 bg-teal-50" aria-live="polite">
              ...
              <AlertDescription>
                <span className="mt-2 flex flex-wrap gap-2">{defaultPositionCheck.positions.map((position) => <Badge
                    key={position.code} variant="secondary">{position.name}</Badge>)}</span>
                <Link className={buttonVariants({className: "mt-3", size: "sm", variant: "outline"})}
                      href={`/companies/${defaultPositionCheck.company.code}/positions`}>직위 관리</Link>
              </AlertDescription>
            </Alert>
            ...
            <Alert className="mt-6 border-amber-300 bg-amber-50" aria-live="polite">
              ...
              <AlertDescription>
                ...
                <Link className={buttonVariants({className: "mt-3 block", size: "sm", variant: "outline"})}
                      href={`/companies/${defaultPositionCheck.company.code}/positions`}>직위 관리</Link>
              </AlertDescription>
            </Alert>
        ) : null}
        ...
        <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="grid gap-4 border-b border-slate-200 p-4 sm:grid-cols-[minmax(14rem,1fr)_12rem_12rem_8rem]">
            <div className="space-y-2"><Label htmlFor="company-search">회사 검색</Label><Input id="company-search"
                                                                                           onChange={(event) => setSearchDraft({
                                                                                             source: querySearch,
                                                                                             value: event.target.value
                                                                                           })}
                                                                                           placeholder="코드, 회사명 또는 도메인"
                                                                                           value={search}/></div>
            <Filter label="상태" value={status ?? ""}
                    onChange={(value) => replaceQuery({status: value || null, page: null})}
                    options={[["", "전체"], ["ACTIVE", "활성"], ["INACTIVE", "비활성"]]}/>
            <Filter label="정렬" value={sort} onChange={(value) => replaceQuery({sort: value, page: null})}
                    options={[["code", "코드"], ["name", "회사명"], ["status", "상태"]]}/>
            <Filter label="페이지 크기" value={String(size)} onChange={(value) => replaceQuery({size: value, page: null})}
          ...
          <div aria-busy={loading} aria-live="polite" className="min-h-52 overflow-x-auto">
            {loading ? <div className="space-y-3 p-5"><span
                    className="sr-only">회사 목록을 불러오는 중입니다.</span>{[1, 2, 3].map((item) => <Skeleton className="h-10 w-full"
                                                                                                   key={item}/>)}</div>
                : error ? <div className="p-6 text-center"><p className="text-sm text-red-700">{error}</p><Button
                    ...
                    : result.content.length === 0 ?
                        <p className="p-12 text-center text-sm text-slate-500">등록된 회사가 없습니다.</p>
                        : <Table><TableHeader><TableRow><TableHead>코드</TableHead><TableHead>회사명</TableHead><TableHead>이메일
                          도메인</TableHead><TableHead>상태</TableHead><TableHead
                            className="text-right">작업</TableHead></TableRow></TableHeader><TableBody>{result.content.map((company) =>
                            <TableRow key={company.code}><TableCell
                                className="font-mono font-medium">{company.code}</TableCell><TableCell>{company.name}</TableCell><TableCell>{company.emailDomain}</TableCell><TableCell><Badge
                                variant={company.status === "ACTIVE" ? "default" : "secondary"}>{company.status === "ACTIVE" ? "활성" : "비활성"}</Badge></TableCell><TableCell
                                className="space-x-2 text-right"><Button onClick={() => setEditing(company)} size="sm"
                                                                         variant="outline">{company.code} 수정</Button><Link
                                className={buttonVariants({size: "sm", variant: "ghost"})}
                                href={`/companies/${company.code}/positions`}>직위</Link></TableCell></TableRow>)}</TableBody></Table>}
          </div>
          <Pagination page={result.page} totalPages={result.totalPages} totalElements={result.totalElements}
                      onPage={(next) => replaceQuery({page: String(next)})}/>
        </div>
        {editing !== undefined ?
            <CompanyForm company={editing} onOpenChange={(open) => !open && setEditing(undefined)} onSaved={handleSaved}
                         open/> : null}
      </section>
  );
}
...
function Pagination({page, totalPages, totalElements, onPage}: {
  ...
  return <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3"><p
      className="text-sm text-slate-500">총 {totalElements.toLocaleString()}개
    · {totalPages === 0 ? 0 : page + 1}/{totalPages} 페이지</p>
    <div className="flex gap-1"><Button disabled={page === 0 || totalPages === 0} onClick={() => onPage(0)} size="sm"
                                        variant="outline">처음</Button><Button disabled={page === 0 || totalPages === 0}
                                                                             onClick={() => onPage(page - 1)} size="sm"
                                                                             variant="outline">이전</Button><Button
        disabled={page >= last || totalPages === 0} onClick={() => onPage(page + 1)} size="sm"
        variant="outline">다음</Button><Button disabled={page >= last || totalPages === 0} onClick={() => onPage(last)}
                                             size="sm" variant="outline">마지막</Button></div>
  </div>;
}
