...
export function CompanySwitcher({ pathname, selectedCompanyCode, fixedCompanyCode }: Props) {
  ...
  return (
    <div className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-2 shadow-sm sm:w-auto">
      ...
      <Input className="mb-2 h-8" id="company-switcher-search" onChange={(event) => setSearch(event.target.value)} placeholder="회사 검색" value={search} />
      ...
      >
        ...
        {selectedCode && !options.some((company) => company.code === selectedCode) ? <option value={selectedCode}>{selectedCode} · 확인 중</option> : null}
        {options.map((company) => <option key={company.code} value={company.code}>{company.name} ({company.code}){company.status === "INACTIVE" ? " · 비활성" : ""}</option>)}
      </select>
      <div className="mt-2 flex items-center justify-between gap-2">
        <Button aria-label="이전 회사 페이지" disabled={page === 0 || loading} onClick={() => setPage((value) => Math.max(0, value - 1))} size="sm" variant="ghost">이전</Button>
        <span className="text-xs text-slate-500">{result.totalPages === 0 ? 0 : result.page + 1} / {result.totalPages} 페이지</span>
        <Button aria-label="다음 회사 페이지" disabled={page + 1 >= result.totalPages || loading} onClick={() => setPage((value) => value + 1)} size="sm" variant="ghost">다음</Button>
      </div>
